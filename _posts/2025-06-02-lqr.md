...
$$
J(\mathbf{x}_0,\mathbf{u}(t), t_0) = \frac{1}{2} \mathbf{x}^\top(t_f) \mathbf{S} \mathbf{x}(t_f) + \frac{1}{2} \int_{t_0}^{t_f} \left[ \mathbf{x}^\top(t) \mathbf{Q} \mathbf{x}(t) + \mathbf{u}^\top(t) \mathbf{R} \mathbf{u}(t) \right] dt
$$
...
上式对 $$ t_0 $$ 求偏导，得 **HBJ 方程**：

$$
\frac{\partial V(\mathbf{x}_0, t_0) }{\partial t_0} = - \min_{\mathbf{u}(t_0)} \left[ \mathbf{x}_0^\top \mathbf{Q} \mathbf{x}_0 + \mathbf{u}^\top \mathbf{R} \mathbf{u}(t_0) + \nabla_{\mathbf{x}_0}V^{\top}(\mathbf{x}_0, t_0) (\mathbf{A} \mathbf{x}_0 + \mathbf{B} \mathbf{u}(t_0))  \right]
$$
...
$ \mathbf{P}(t) \in \mathbb{R}^{n \times n} $ 是从**终端时间 $ t_f $** 向**当前时间 $ t $** 反向解出的 **Riccati 微分方程**（Differential Riccati Equation）的解，即求解下面的微分方程得到：
