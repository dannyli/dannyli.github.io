...
有多种经典的方法可以确定 $\mathbf{K}$，例如极点配置法（MATLAB 函数 `place`）。
 ...
 LQR 本质是通过最优控制的方法确定 $\mathbf{K}$。对于无限时间区间连续时间型 LQR，其控制目标是最小化以下泛函--积分型成本函数（Cost function）：

$$
J = \int_0^\infty \ell(\mathbf{x}(t), \mathbf{u}(t)) dt =\int_0^\infty  \left[ \mathbf{x}(t)^\top \mathbf{Q} \mathbf{x}(t) + \mathbf{u}(t)^\top \mathbf{R} \mathbf{u}(t) \right] dt
$$
...
首先，定义**值函数（Value Function, Critic）**为成本函数的最小值：

$$
V(\mathbf{x}) = \min_{\mathbf{u}} J(\mathbf{x},\mathbf{u}) = \min_{\mathbf{u}} \int_0^\infty  \left( \mathbf{x}(t)^\top \mathbf{Q} \mathbf{x}(t) + \mathbf{u}(t)^\top \mathbf{R} \mathbf{u}(t) \right) dt
$$

对上式两边求时间导数，得到无限时间区间的**哈密顿-雅可比-贝尔曼（Hamilton-Jacobi-Bellman, HJB）方程**：

$$
0 =  \min_{\mathbf{u}(t)} \left[\mathbf{x}^\top \mathbf{Q} \mathbf{x} +\mathbf{u}(t)^\top \mathbf{R} \mathbf{u}(t) + \nabla_{\mathbf{x}}V(\mathbf{x})^{\top} \cdot (\mathbf{A} \mathbf{x}(t) + \mathbf{B} \mathbf{u}(t))  \right]
$$

定义**协态（Costate）**$\boldsymbol{\lambda} \in \mathbb{R}^n$ ，表示状态 $\mathbf{x}(t)$ 的一个“无穷小变化”会对从 $ t $ 到终点的最优总代价造成多大的影响：
...
$$
0 =  \min_{\mathbf{u}(t)} \left[\mathcal{H}(\mathbf{x}, \mathbf{u}, \boldsymbol{\lambda})  \right]
$$

定义[哈密顿量（Hamiltonian）](https://zh.wikipedia.org/wiki/%E5%93%88%E5%AF%86%E9%A0%93%E9%87%8F_(%E6%9C%80%E4%BD%B3%E6%8E%A7%E5%88%B6))为：

$$
\begin{align}
\mathcal{H}(\mathbf{x}, \mathbf{u}, \boldsymbol{\lambda}) &= \ell(\mathbf{x}(t), \mathbf{u}(t)) + \boldsymbol{\lambda}^{\top} f(\mathbf{x}(t), \mathbf{u}(t)) \nonumber \\ 
&=\mathbf{x}^{\top} \mathbf{Q} \mathbf{x} + \mathbf{u}^{\top} \mathbf{R} \mathbf{u} + \boldsymbol{\lambda}^{\top} (\mathbf{A} \mathbf{x} + \mathbf{B} \mathbf{u}) \label{eq_hamiltonian}
\end{align}
$$

而 **$$\mathcal{Q}$$ 函数**和 Hamiltonian 密切相关。连续时间下，其定义为：

$$
\mathcal{Q}(\mathbf{x}, \mathbf{u}) = \mathcal{H}(\mathbf{x}, \mathbf{u}, \boldsymbol{\lambda}= \nabla_{\mathbf{x}}V(\mathbf{x})) = \ell(\mathbf{x}(t), \mathbf{u}(t)) + \nabla_{\mathbf{x}}V(\mathbf{x})^{\top} f(\mathbf{x}(t), \mathbf{u}(t))
$$
...
其中 $\mathbf{P} \in \mathbb{R}^{n \times n}$ 是一对称常值变换方阵（强化学习中学习 Critic 即学习 $$\mathbf{P}$$）。因此，协态是状态的线性变换：

$$
\nabla_{\mathbf{x}}V(\mathbf{x}) = \boldsymbol{\lambda} =  2 \mathbf{P} \mathbf{x}(t)  \label{eq:lambda}
$$

在最优控制的条件下，对应的**一阶条件（First-order conditions, FOC）**为：
...
2. 协态方程：
	...
	$$
	\dot{\boldsymbol{\lambda}}(t) = -\frac{\partial \mathcal{H}}{\partial \mathbf{x}} = -2\mathbf{Q} \mathbf{x}(t) - \mathbf{A}^{\top} \boldsymbol{\lambda}(t) \label{eq:costate_eq}
	$$

3. 最优性条件（Optimality condition）：

	$$
	\frac{\partial \mathcal{H}}{\partial \mathbf{u}} = 2 \mathbf{R} \mathbf{u}(t) +  \mathbf{B}^{\top} \boldsymbol{\lambda}(t) = 0 \label{eq:optimality}
	$$
...
$$
\begin{align}
\mathbf{u}^*(t) &= -\frac{1}{2} \mathbf{R}^{-1}  \mathbf{B}^{\top} \boldsymbol{\lambda}(t) = - (\mathbf{R}^{-1} \mathbf{B}^\top \mathbf{P}) \mathbf{x}(t) \label{eq_optu3}
\end{align}
$$

为了求得 $$\mathbf{P}$$，将 $$\eqref{eq:lambda}$$ 带入协态方程 $$\eqref{eq:costate_eq}$$，得：
